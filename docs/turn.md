...
# Optional: disable automatic captcha solving and delegate captcha URL to a helper.
# The command must print VK success_token to stdout. The captcha URL is also
# exposed in VK_CAPTCHA_URL; {url} is replaced with the URL if present.
VKAutoCaptcha = false
VKCaptchaCommand = powershell -ExecutionPolicy Bypass -File C:\wg-turn\solve-vk-captcha.ps1
...
- `VKAutoCaptcha`: set to `false` to skip automatic VK captcha solving and use
  manual solving through `VKCaptchaCommand`. Defaults to `true`.
- `VKCaptchaCommand`: command used for manual VK captcha solving. The command
  receives the captcha URL in the `VK_CAPTCHA_URL` environment variable, may use
  `{url}` as an inline placeholder, and must print the VK `success_token` to stdout.
