...
func (conf *Config) ToWgQuick() string {
	...
	if conf.Turn.Enabled {
		...
		if conf.Turn.VKAutoCaptchaSet {
			output.WriteString(fmt.Sprintf("VKAutoCaptcha = %t\n", conf.Turn.VKAutoCaptcha))
		}
		if conf.Turn.VKCaptchaCommand != "" {
			output.WriteString(fmt.Sprintf("VKCaptchaCommand = %s\n", conf.Turn.VKCaptchaCommand))
		}
