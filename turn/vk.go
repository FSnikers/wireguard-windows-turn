...
// Custom dial context for tls-client. On Windows the proxy is started before
// the WireGuard adapter is brought up, so the normal system route is used here.
func getCustomDialContext(ctx context.Context, network, addr string) (net.Conn, error) {
	dialer := getCustomNetDialer()
	return dialer.DialContext(ctx, network, addr)
}

type vkCredentialOptions struct {
	AutoCaptcha    bool
	CaptchaCommand string
}
...
func fetchVkCreds(ctx context.Context, link string) (string, string, string, error) {
	return fetchVkCredsWithOptions(ctx, link, vkCredentialOptions{AutoCaptcha: true})
}

func fetchVkCredsWithOptions(ctx context.Context, link string, options vkCredentialOptions) (string, string, string, error) {
	...
	for _, creds := range vkCredentialsList {
		user, pass, addr, err := getTokenChain(ctx, link, creds, client, profile, options)
		if err == nil {
...
func getTokenChain(ctx context.Context, link string, creds VKCredentials, client tlsclient.HttpClient, profile Profile, options vkCredentialOptions) (string, string, string, error) {
	...
	manualCaptcha := true
	autoCaptchaSliderPOC := options.AutoCaptcha
	streamID := 0
			...
			if captchaErr != nil && captchaErr.IsCaptchaError() {
				...
				case captchaSolveModeAuto:
					if !options.AutoCaptcha {
						turnLog("[Captcha] Auto solving disabled; using manual captcha command")
						successToken, solveErr = solveVkCaptchaManual(ctx, captchaErr.RedirectURI, options.CaptchaCommand)
						break
					}
				...
				case captchaSolveModeManual:
					turnLog("[STREAM %d] [Captcha] Running manual captcha command", streamID)
					successToken, solveErr = solveVkCaptchaManual(ctx, captchaErr.RedirectURI, options.CaptchaCommand)
				}
